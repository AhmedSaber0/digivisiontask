...
class PostsFragment : BaseFragment<
    ...
    override val fragmentTheme: Int = R.style.Theme_DigiVisionTask_posts
    ...
    private fun setupPostsRecyclerView() {
        setupAdapters()
        binding.postsRecyclerView.onEndReached { postAction(PostsViewAction.LoadNextPage) }
        binding.postsRecyclerView.adapter = postsAdapter
    }

    private fun setupAdapters() {
        postsAdapter = PostsAdapter {
            postAction(PostsViewAction.OpenPostDetails(it.title, it.url))
        }
    }
